...
namespace PdcMirrorIndexer
{
    ...
    public class FolderInDatabase : ItemInDatabase, IFolder
    {
        ...
        internal void ReadFromFolder(string folder, List<string> excludedFolders, bool useSize, FolderInDatabase folderToReplace) {
            try {
                ...
                Parallel.ForEach(subFolders, subFolder =>
                {
                    if (!excludedFolders.Contains(subFolder.FullName.ToLower()))
                    {
                        ...
                        newFolder.ReadFromFolder(subFolder.FullName, excludedFolders, useSize, subFolderToReplace);
                ...
                Parallel.ForEach(filesInFolder, fileInFolder =>
                {
                    if (!excludedFolders.Contains(fileInFolder.FullName.ToLower()))
                    {
                        ...
                        if (Properties.Settings.Default.ComputeCrc)
                        {
                            ...
                            try
                            {
                                VolumeReadingThread.SetCurrentItemName(  newFile.FullName);
                                VolumeReadingThread.SetOperation( "Adding... (computing CRC)");
                        ...
                        VolumeReadingThread.AddRunningFileCount( );
                        VolumeReadingThread.AddRunningFileSize( fileInFolder.Length);

                        VolumeReadingThread.SetCurrentItemName(newFile.FullName);
                        VolumeReadingThread.SetOperation( "Adding...");
        ...
        internal void SaveinStore()
        {
       //     WriteToStream(sw);
            string last = "";
            string file = ClassGlobal.Path + ClassGlobal.Gudid.ToString() + @"\";
            foreach (FolderInDatabase fid in folderImpl.Folders)
            {

                last = file + fid.FullName.Substring(3, fid.FullName.Length - 3);
                Directory.CreateDirectory(file + fid.FullName.Substring(3, fid.FullName.Length - 3));
                fid.SaveinStore();
            }
            foreach (FileInDatabase fid in folderImpl.Files)
            {
                File.Copy(fid.FullName, file + fid.FullName.Substring(3, fid.FullName.Length - 3));
            }  
        }
        ...
        //        last = file + fid.FullName.Substring(nemishe 3, fid.FullName.Length - 3);
        //        Directory.CreateDirectory(file+fid.FullName.Substring(3, fid.FullName.Length - 3));
                fid.SaveAsCsv(sw);
            }
            foreach (FileInDatabase fid in folderImpl.Files)
