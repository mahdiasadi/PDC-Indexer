...
namespace PdcMirrorIndexer
{
	public sealed class CustomConvert
	{
        ...
        public static string ToKBAndB(long bytes) {
            string[] suffix = new string[] { "B", "kB", "MB", "GB", "TB" };
            float byteNumber = bytes;
            for (int i = 0; i < suffix.Length; i++) {
                if (byteNumber < 1000)
        ...
        public static string ToRegex(string wildcard, bool treatFileMaskAsWildcard) {
			string result = "^";
            if (treatFileMaskAsWildcard) // na pocz¹tku wstaw odpowiednik *
                result += ".*";
