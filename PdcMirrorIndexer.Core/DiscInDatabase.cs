...
namespace PdcMirrorIndexer
{
	...
	public class DiscInDatabase : FolderInDatabase, IComparable<DiscInDatabase> {
        ...
        internal override string GetCsvLine() {
            return base.GetCsvLine() + "," + DriveFormat + "," + DriveType.ToString() + "," + TotalFreeSpace.ToString() + "," + TotalSize.ToString() + "," + VolumeLabel + ","+Gudid ;
        }
        ...
        internal void ReadFromDrive(string drive, List<string> elementsToRead, ref long runningFileCount, ref long runningFileSize, ref string currentItemName, ref string operation, bool useSize, DiscInDatabase discToReplace) {
            ReadFromFolder(drive, elementsToRead, useSize, discToReplace);
